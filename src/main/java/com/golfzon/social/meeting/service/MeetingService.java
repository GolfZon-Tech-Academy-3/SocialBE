...
public class MeetingService {
    ...
    //모임 개설하기
    public String postMeeting(MeetingDto.Request request, Member member) {
        ...
        meetingMemberRePository.save(new MeetingMember(member, meeting, "leader", true));        //member
        List<Member> members = memberRepository.findAllByRole("master");
        for (Member m:members) {
            meetingMemberRePository.save(new MeetingMember(m, meeting, "master", true));
        }
        //알림 푸쉬
    ...
    //모임 상세보기
    public MeetingDto.Response getMeeting(Long meetingId, Member member) {
        //모임 아이디에 해당하는 모임 찾기
        ...
        return new MeetingDto.Response(new MeetingDto.Info(meeting), meetingMember.getRole());
    }

    //모임 가입하기
    public MeetingDto.Role signupMeeting(Long meetingId, Member member) {
        String role = "member";
        //모임 아이디에 해당하는 모임 찾기
        Meeting meeting = meetingRepository.findById(meetingId).orElseThrow(
                () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "해당하는 모임 아이디가 없습니다."));
        ...
        if(meetingMember != null){
            ...
            if(meeting.getSecret()) {
                role = "waiting";
                meetingMemberRePository.save(new MeetingMember(member, meeting, role, false));
                // 가입신청 알림 푸쉬
            } else {
                meetingMemberRePository.save(new MeetingMember(member, meeting, role, true));
                // 가입완료 알림 푸쉬
            }
        }
    ...
    //모임 수정하기
    public String updateMeeting(Long meetingId, MeetingDto.Request request, Member member) {
