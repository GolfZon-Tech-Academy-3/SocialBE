...
public class RoundS3Service {
    ...
    private final String bucket = "social2-bucket";
    ...
    public String update(Long roundId, MultipartFile file) {
        String imageUrl;
        List<String> imageUrls = new ArrayList<>(Arrays.asList(MemberService.profileImages));
        Optional<Round> round = roundRepository.findById(roundId);
        if (round.isPresent()) {
            String savedImage = round.get().getImageUrl().replace("https://spacez3.s3.ap-northeast-2.amazonaws.com/", "");
